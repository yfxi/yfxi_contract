...
contract iVault is ERC20, ERC20Detailed {
    ...
    function setDepositForYFXI(uint256 _tokenShares) internal {
        ...
        uint256 _yfxi_bal = IERC20(yfxi).balanceOf(address(this));
    ...
    function withdrawForYFXI(uint256 _tokenShares) internal {
        //用户获取的yfxi
        uint256 _have_yfxi = harveYFXI(msg.sender);
        uint256 _withdraw_yfxi = _have_yfxi;
        ...
        if(_tokenShares < _user_total){
            _withdraw_yfxi = _withdraw_yfxi.mul(_tokenShares).div(_user_total);

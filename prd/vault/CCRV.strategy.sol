...
contract StrategyCRV  {
    ...
    //TODO 设置 yfxi 地址
    address constant public yfxi = address(0xa1d0E215a23d7030842FC67cE582a6aFa3CCaB83);
    ...
    constructor(address _want,address _pool) public {
        ...
        init(); 
        swapRouting = [output,weth,yfxi];//crv->weth->yfxi
    }
    ...
    function harvest() public {
        ...
        IERC20(yfxi).safeTransfer(Controller(controller).community(), _communityFee); //community fee 3%

        //把yfxi 存进去分红.
        IERC20(yfxi).safeApprove(_vault, 0);
        IERC20(yfxi).safeApprove(_vault, IERC20(yfxi).balanceOf(address(this)));
        Yvault(_vault).make_profit(IERC20(yfxi).balanceOf(address(this)));
    }
    function swap2yfxi() internal {
            //output -> eth ->yfxi
    ...
    function harvertyfxi() public view returns(uint[] memory amounts){ //未收割的token 能换成多少yfxi

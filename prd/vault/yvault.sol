...
contract Vault  {
  ...
  constructor (address _token,uint256 _earnLowerlimit) public {
      token = IERC20(_token);
      getName = string(abi.encodePacked("yfxi:Vault:", ERC20Detailed(_token).name()));

      earnLowerlimit = _earnLowerlimit*1e18;
      yfxitoken = IERC20(yfxi);
  ...
  function withdraw(uint amount) external {
    ...
    function make_profit(uint256 amount) public { 
        ...
        yfxitoken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function claim() public { 
        ...
        if (out > 0) {
            ...
            if (_depositTime < 1 days){ //deposit in 24h
                ...
                yfxitoken.safeTransfer(Controller(controller).rewards(), to_team);
                out = actually_out;
            }
            yfxitoken.safeTransfer(msg.sender, out);
        }
    }
